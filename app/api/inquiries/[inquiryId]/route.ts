...
export async function GET(req: NextRequest, { params }: { params: Promise<{ inquiryId: string }> }) {
  ...
  const { data, error } = await supabaseAdmin
    .from('inquiries')
    .select(`inquiry_id, category, title, content, created_at, inquiry_replies(id, content, created_at)`)
    .eq('inquiry_id', id)
  ...
  type Row = {
    ...
    inquiry_replies: { id: number; content: string; created_at: string }[] | null;
  };
  const r = data as unknown as Row;
  const replies = r.inquiry_replies ?? [];

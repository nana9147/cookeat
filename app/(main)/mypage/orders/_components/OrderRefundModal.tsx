...
export default function OrderRefundModal({ orderId, onClose, onSuccess }: Props) {
  ...
  const handleSubmit = async () => {
    setSubmitting(true);
    try {
      await api.post(`/orders/${orderId}/refund`, { reason: reason.trim() || undefined });
      ...
      const message =
        e && typeof e === 'object' && 'response' in e
          ...
          : undefined;
      toast.error(message ?? '환불 신청에 실패했습니다.');
    } finally {
      setSubmitting(false);
    }
  };
    ...
    >
      <div className="relative w-full sm:max-w-sm bg-white rounded-t-3xl sm:rounded-2xl shadow-xl">
        <div className="flex items-center justify-between px-5 pt-5 pb-4 border-b border-border">
          <h2 className="font-bold text-dark-text">환불 신청</h2>
        ...
        <div className="px-5 py-4 flex flex-col gap-3">
          <p className="text-xs text-gray-text">환불 사유를 입력해주세요. (선택)</p>
          <Textarea
            value={reason}

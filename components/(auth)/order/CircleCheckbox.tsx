...
type Props = { checked: boolean; onChange?: () => void; className?: string };

export default function CircleCheckbox({ checked, onChange, className = '' }: Props) {
  ...
  if (onChange) {
    return (
      <button type="button" onClick={onChange} className={base}>
        {checked && checkmark}
      </button>
    );
  }
